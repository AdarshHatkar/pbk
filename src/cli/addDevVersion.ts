...
export async function addDevVersion(targetDir?: string): Promise<void> {
  try {
    ...
    const originalVersion = packageJson.dependencies?.['@primexop/pbk'] || packageJson.devDependencies?.['@primexop/pbk'];
    
    // Determine if it's in dependencies or devDependencies
    let dependencyType = 'dependencies';
    if (!packageJson.dependencies?.['@primexop/pbk'] && packageJson.devDependencies?.['@primexop/pbk']) {
    ...
    packageJson[dependencyType]['@primexop/pbk'] = pbkPath;
    
    // Save the updated package.json
    fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));
    
    console.log(`Updated @primexop/pbk to use local version: ${pbkPath}`);
    if (originalVersion) {
    ...
    console.log('Successfully installed the local development version of @primexop/pbk');
  } catch (error) {
